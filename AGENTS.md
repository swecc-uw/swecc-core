...
**Env-author CLI (PyPI):** There is only one published package: **`swecc-mesocosm`** (`packages/swecc-mesocosm/`). It is not a separate product from “bench” — `pip install swecc-mesocosm` installs both the `mesocosm` command (Typer, bench-api HTTP) and the `bench` command (`bench init`, `bench run local`, auth, submit). The library `bench_common` under `services/bench/common/` is bundled into that wheel; **do not** publish or document a separate PyPI name like `swecc-bench`. See `packages/swecc-mesocosm/PACKAGING.md`.
