...
class ReadingGroupConfig:
    ...
    book_url: str = (
        "https://discord.com/channels/960050427657863218/1352172643469627443/1352178235999387699"
    )
    ...
    chapter_schedule: List[Tuple[int, ...]] = field(
        default_factory=lambda: [
            (0, 1),  # week 1: ch. 1, 2
            (2,),  # week 2: ch. 3
            (3,),  # week 3: ch. 4
...
@app_commands.command(
    name="reading",
    description="Create a reading group thread for a specific week and date",
)
...
async def create_reading_group_thread(
    interaction: discord.Interaction, week: int, date: str, time: str, location: str
):
    ...
    if officer_role not in interaction.user.roles:
        await interaction.followup.send(
            "You do not have permission to create reading group threads.",
            ephemeral=True,
        )
        return
    ...
    if week < 1 or week > len(config.chapter_schedule):
        await interaction.followup.send(
            f"Invalid week. Please choose a week between 1 and {len(config.chapter_schedule)}.",
            ephemeral=True,
        )
        return
    ...
    try:
        ...
        error_msg = (
            ...
            if "date" in str(e)
            else "Invalid time format. Use HH:MM."
        )
    ...
    try:
        ...
        await thread.send(
            "Feel free to share your thoughts, questions, and insights on this week's reading."
        )

        await interaction.followup.send(
            f"Successfully posted reading assignment for Week {week}.", ephemeral=True
        )

        admin_channel = interaction.guild.get_channel(bot_context.admin_channel)
        if admin_channel:
            await admin_channel.send(
                f"User {interaction.user.mention} created reading assignment for Week {week}."
            )
