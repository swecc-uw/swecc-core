...
class AdventOfCodeAPI:
    def __init__(self):
        ...
        self.leaderboard_url = (
            f"https://adventofcode.com/{self.year}/leaderboard/private/view/{self.leaderboard_id}"
        )
        self.headers = {
    ...
    async def get_leaderboard(self):
        ...
        if (
            self.cache["data"]
            and self.cache["last_accessed"]
            and (now - self.cache["last_accessed"]) <= timedelta(minutes=16)
        ):
