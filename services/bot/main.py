...
async def on_message(message):
    ...
    await mq.producers.publish_message_event(
        MessageEvent(discord_id=member.id, channel_id=message.channel.id, content=message.content)
    )
...
async def on_thread_create(thread):
    if thread.guild.id == bot_context.swecc_server:
        ...
        if thread.parent_id == bot_context.resume_channel:
            ...
            if not message.attachments or not message.attachments[0].content_type.startswith(
                "image"
            ):
                try:
                ...
                if not message.attachments:
                    await message.author.send(
                        f"Hello {message.author.mention}, your resume post in {channelName} was deleted because it didn't contain a screenshot of your resume. Please try again."
                    )
                else:
                    await message.author.send(
                        f"Hello {message.author.mention}, your resume post in {channelName} was deleted because it didn't contain a screenshot of your resume. Please try again with a screenshot instead of {message.attachments[0].content_type}."
                    )

                await bot_context.log(
                    message,
                    f"{message.author.mention}'s resume post in {channelName} was deleted. File type: {message.attachments[0].content_type if message.attachments else 'none'}",
                )
...
async def on_raw_reaction_add(payload):
    ...
    await mq.producers.publish_reaction_add_event(
        ReactionEvent(
