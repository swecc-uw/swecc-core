...
async def assert_dev_env_access(env_id: str, principal: Principal) -> DevEnvRow:
    try:
        ...
        raise HTTPException(status_code=404, detail=f"Environment '{env_id}' not found") from exc
    if not await can_access_dev_env(row, principal):
        raise HTTPException(status_code=403, detail="Not allowed to access this environment")
    return row
