...
def write_to_file(
    data: List[DynamoHealthMetric],
    output_file: str = "compacted.py",
    variable_name: str = "compacted_data",
):
...
filtered_data: List[DynamoHealthMetric] = [
    item for item in data if datetime.fromisoformat(item["timestamp"]) < two_weeks_ago
]
